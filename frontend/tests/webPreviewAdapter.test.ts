...
import type { SettingsSnapshot, SettingWriteResult } from '../apps/settings/src/types.ts'
import { browserPreviewAdapter } from '../apps/settings/src/services/webPreviewAdapter.ts'
...
test('preview adapter supports the real settings route contract', async () => {
  ...
  assert.equal(snapshot.data?.appearance.ui_scale, 'auto')
})
