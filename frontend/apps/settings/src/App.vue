...
import type { SettingsSnapshot, ThemeMode, UiScale } from './types'
...
const appliedUiScale = ref<UiScale>('auto')
const uiScaleRestartPending = ref(false)
...
const uiScaleChanged = computed(() =>
  Boolean(
    original.value
      && draft.value
      && original.value.appearance.ui_scale !== draft.value.appearance.ui_scale,
  ),
)
const uiScaleRequiresRestart = computed(
  () => uiScaleChanged.value || uiScaleRestartPending.value,
)
...
const accentStyle = computed(() => ({
  '--accent': draft.value?.appearance.accent_color || '#006874',
}))
...
watch([effectiveTheme, accentStyle], () => {
  ...
  document.documentElement.style.setProperty(
    '--accent',
    draft.value?.appearance.accent_color || '#006874',
  )
}, { immediate: true })
...
async function loadSettings() {
  ...
  try {
    ...
    appliedUiScale.value = snapshot.appearance.ui_scale || 'auto'
    uiScaleRestartPending.value = false
...
async function saveSettings() {
  ...
  try {
    ...
    uiScaleRestartPending.value =
      draft.value.appearance.ui_scale !== appliedUiScale.value
...
const accentSwatches = ['#006874', '#006A6A', '#4F6354', '#7A5900', '#6750A4']
</script>
    ...
    <div v-else-if="draft" class="workspace">
      ...
      <div class="settings-content">
        ...
        <section id="appearance" class="settings-section">
          ...
          <label class="setting-row select-row">
            <span>
              <strong>界面缩放</strong>
              <small>自动模式跟随系统 DPI；手动档位重启后生效</small>
            </span>
            <select v-model="draft.appearance.ui_scale" class="select-control">
      ...
      <footer class="save-bar">
        <span class="save-summary">
          ...
          <small v-if="uiScaleRequiresRestart">界面缩放将在重启后应用</small>
        </span>
