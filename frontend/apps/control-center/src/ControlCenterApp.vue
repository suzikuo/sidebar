...
const uiScaleChanged = computed(() =>
  String(draftSettings.value.appearance?.ui_scale || 'auto')
    !== String(originalSettings.value.appearance?.ui_scale || 'auto'),
)
...
const accentColor = computed(() => String(draftSettings.value.appearance?.accent_color || '#006874'))
...
async function loadAll() {
  ...
  try {
    ...
    appliedUiScale.value = String(settings.appearance?.ui_scale || 'auto')
...
async function saveSettings() {
  ...
  try {
    ...
    uiScaleRestartPending.value =
      String(draftSettings.value.appearance?.ui_scale || 'auto') !== appliedUiScale.value
...
async function resetSettings(category?: string) {
  ...
  try {
    ...
    draftSettings.value = clone(settings)
    uiScaleRestartPending.value =
      String(settings.appearance?.ui_scale || 'auto') !== appliedUiScale.value
    ...
    <main class="main-workspace">
      ...
      <div v-else class="workspace-content">
        ...
        <section v-else-if="activeView === 'settings'" class="settings-view">
          <div v-if="uiScaleRequiresRestart" class="settings-restart-banner">
            <div><RefreshCw :size="18" /><span>界面缩放将在重启后应用</span></div>
            <button class="button primary" type="button" @click="restartApplication">立即重启</button>
          </div>
