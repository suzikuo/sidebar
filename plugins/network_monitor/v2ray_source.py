...
class V2RayCollector:
    ...
    def collect(self, value=None):
        ...
        signature = (
            metrics_config.host,
            metrics_config.port,
            metrics_config.timeout_seconds,
        )
        ...
        if now < self._retry_at:
            return self._disconnected_snapshot(config, self._last_error)
        try:
            ...
            base_delay = max(1.0, config["refresh_interval_ms"] / 1000.0)
            retry_delay = min(
                MAX_RETRY_DELAY_SECONDS,
                base_delay * (2 ** min(self._failure_count - 1, 4)),
            )
            self._retry_at = now + retry_delay
    ...
    def _disconnected_snapshot(self, config, error):
        return V2RaySnapshot(
            rates=None,
            enabled=True,
            connected=False,
