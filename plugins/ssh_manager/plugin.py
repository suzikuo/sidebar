...
class SSHManagerPlugin(PluginBase):
    ...
    def get_card_widget(self) -> QWidget:
        if self.ui_widget is None:
            from .views import SSHManagerWidget
    ...
    def _command_catalog(self, payload, request_context):
        ...
        commands = []
        for connection in self.db.list_connections():
            commands.append(
                {
                    "id": str(connection["id"]),
                    "name": connection["name"],
                    "subtitle": f"{connection['user']}@{connection['host']}:{connection['port']}",
                    "category": "SSH",
    ...
    def _command_execute(self, payload, request_context):
        del request_context
        connection_id = int(payload.get("id"))
        if self.db.get_connection(connection_id) is None:
            raise ValueError("SSH 连接不存在。")
        self.connect_ssh(connection_id)
        return {"executed": True}

    def _backup_snapshot(self, payload, request_context):
        del payload, request_context
        fields = ("name", "host", "user", "port", "pem_path", "remarks", "color")
        return {
            "version": 1,
            "connections": [
                {field: connection[field] for field in fields}
                for connection in self.db.list_connections()
            ],
    ...
    def _backup_restore(self, payload, request_context):
        ...
        if not isinstance(connections, list) or not all(
            ...
            raise ValueError("SSH 备份格式无效。")
        restored = self.db.replace_connections(connections)
        if self.ui_widget is not None:
            self.ui_widget._refresh_list()
        return {"restored": restored, "privateKeysIncluded": False}
