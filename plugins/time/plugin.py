...
from .logic import AlarmManager
from .views import ClockWidget, TimeSettingsWidget
from .desktop_clock import DesktopClockWidget
...
class TimePlugin(PluginBase):
    ...
    def on_load(self):
        ...
        self._sync_desktop_clock(config)
    ...
    def on_unload(self):
        ...
        if self._clock_widget is not None:
            self._clock_widget.timer.stop()
        self._clock_widget = None
        self._destroy_desktop_clock()
    ...
    def _sync_desktop_clock(self, config: dict):
        if not config.get("show_desktop", False):
            self._destroy_desktop_clock()
            return
        if self._desktop_clock is None:
            self._desktop_clock = DesktopClockWidget()
            self._desktop_clock.set_alarm_manager(self._alarm_manager)
            self._desktop_clock.position_changed.connect(
                self._on_desktop_clock_moved
            )
        self._desktop_clock.apply_config(config)
        self._desktop_clock.show()

    def _destroy_desktop_clock(self):
        if self._desktop_clock is None:
            return
        widget = self._desktop_clock
        self._desktop_clock = None
        widget.close()
        widget.deleteLater()
