...
from .models import GatewayDatabase, validate_target_url
...
class GatewayManagerPlugin(PluginBase):
    ...
    def get_card_widget(self) -> QWidget:
        if self.web_widget is None:
            from .web_view import GatewayInterface
    ...
    def stop_gateway(self, gateway_id):
        ...
        return self.runtime.stop_gateway(gateway_id)

    def reload_runtime(self):
        if not self._active_gateway_ids:
            return True

        return self.runtime.start(self._active_runtime_config())
    ...
    def _sync_status_timer(self):
        ...
        if timer is None:
            return
        should_run = bool(self.cloudflare_processes)
