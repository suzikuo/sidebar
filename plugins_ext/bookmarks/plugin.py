...
class BookmarksPlugin(PluginBase):
    ...
    def get_card_widget(self) -> QWidget:
        if self.ui_widget is None:
            from .views import BookmarksWidget
    ...
    def _handle_add_bookmark(self):
        from .dialogs import AddBookmarkDialog
    ...
    def _command_catalog(self, payload, request_context):
        ...
        return {
            "commands": [
                {
                    "id": str(index),
                    "name": bookmark.get("title") or "书签",
                    "subtitle": bookmark.get("url") or "",
                    "category": "书签",
                    "route": "command-execute",
                    "payload": {"url": bookmark.get("url") or ""},
                }
    ...
    def _command_execute(self, payload, request_context):
        ...
        if not url:
            raise ValueError("书签地址为空。")
        self._handle_url_clicked(url)
        return {"executed": True}

    def _backup_snapshot(self, payload, request_context):
        del payload, request_context
        return {"version": 1, "groups": self.groups, "bookmarks": self.bookmarks}

    def _backup_restore(self, payload, request_context):
        del request_context
        groups = payload.get("groups")
        bookmarks = payload.get("bookmarks")
        if not isinstance(groups, list) or not isinstance(bookmarks, list):
            raise ValueError("书签备份格式无效。")
        if not all(isinstance(item, dict) for item in groups + bookmarks):
            raise ValueError("书签备份条目无效。")
        self.groups = [dict(item) for item in groups]
        self.bookmarks = [dict(item) for item in bookmarks]
        self._save_data()
        self._refresh_ui(refresh_groups=True)
        return {"restored": len(bookmarks)}
