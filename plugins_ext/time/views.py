...
from .logic import DAY_NAMES_CN, format_days
...
class ClockWidget(StrongBodyLabel):
    ...
    COUNTDOWN_THRESHOLD = COUNTDOWN_THRESHOLD_SECONDS
    ...
    def _render_clock(self):
        ...
        self._set_style(f"color: {self._color}; background: transparent;")
        current_time = QTime.currentTime().toString(self._format)
        if self._orientation == "top":
            self._set_text(current_time)
        else:
    ...
    def _render_countdown(self, info: dict):
        ...
        self._set_style("color: #FFB900; background: transparent;")
